...
def _derive_artifact_names(config_filename):
    """Derive checkpoint and metrics filenames from the config filename stem.

    config.json        -> baseline_transformer.pt  /  baseline_metrics.json
    config_tuned.json  -> tuned_transformer.pt     /  tuned_metrics.json
    config_foo.json    -> foo_transformer.pt        /  foo_metrics.json
    """
    stem = os.path.splitext(os.path.basename(config_filename))[0]  # e.g. "config_tuned"
    if stem == "config":
        prefix = "baseline"
    elif stem.startswith("config_"):
        prefix = stem[len("config_"):]  # strip leading "config_"
    else:
        prefix = stem
    checkpoint_name = f"{prefix}_transformer.pt"
    metrics_name = f"{prefix}_metrics.json"
    return checkpoint_name, metrics_name, prefix


def train_transformer(config_filename="config.json"):
    checkpoint_name, metrics_name, model_prefix = _derive_artifact_names(config_filename)

    print(f"=== Training Temporal Transformer ===")
    print(f"Config: {config_filename}  |  Checkpoint: {checkpoint_name}  |  Metrics: {metrics_name}")
    ...
    # Resolve optimizer from config (default: Adam)
    ...
    assert "ICULOS" in dynamic_features, "ICULOS must be present in dynamic features"
    ...
    print(f"Optimizer: {optimizer_name}  |  Scheduler: {use_scheduler}")

    # Optionally build LR scheduler
    scheduler = None
    if use_scheduler:
    ...
    for epoch in range(config['epochs']):
        ...
        # Step scheduler on val_auprc if active, and report LR changes
        if scheduler is not None:
            lr_before = optimizer.param_groups[0]['lr']
            scheduler.step(val_auprc)
            lr_after = optimizer.param_groups[0]['lr']
            lr_note = f" | LR: {lr_after:.2e}"
            if lr_after < lr_before:
                lr_note += " (reduced)"
        else:
            lr_note = ""

        print(f"Epoch {epoch+1:02d}: Train Loss: {avg_train_loss:.4f} | Val Loss: {val_loss:.4f} | Val AUPRC: {val_auprc:.4f} | Val AUROC: {val_auroc:.4f}{lr_note}")
        ...
        if val_auprc > best_val_auprc:
            ...
            torch.save(checkpoint_data, os.path.join(artifacts_dir, checkpoint_name))
    ...
    checkpoint = torch.load(os.path.join(artifacts_dir, checkpoint_name), map_location=device)
    ...
    print(f"=== {model_prefix.capitalize()} Transformer Test Results ===")
    print(f"  Test Loss:  {test_loss:.4f}")
    print(f"  Test AUROC: {test_auroc:.4f}")
    print(f"  Test AUPRC: {test_auprc:.4f}")

    # Save comprehensive metrics — includes all fields required by spec
    metrics = {
        "model": f"Transformer {model_prefix.capitalize()} (35-Feature)",
        ...
        "config_source": f"preprocessing_config.json for data schema, {config_filename} for hyperparams"
    }
    metrics_path = os.path.join(artifacts_dir, metrics_name)
    ...
    if HAS_MATPLOTLIB:
        ...
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.title(f'Transformer ({model_prefix.capitalize()}) Training and Validation Loss')
        plt.legend()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Temporal Transformer for Sepsis Prediction")
    parser.add_argument(
        "--config",
        type=str,
        default="config.json",
        help="Config filename (relative to this script's directory). "
             "Default: config.json (64-d baseline). "
             "Example: --config config_tuned.json"
    )
    args = parser.parse_args()
    train_transformer(config_filename=args.config)
